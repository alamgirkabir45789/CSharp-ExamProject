...
{
    public class XMLRepositoryBase<TContext, TEntity, TKey> : IRepository<TEntity, TKey> where TContext :
   XMLSet<TEntity> where TEntity : class
    {
        ...
        public bool Update(TEntity model)
        {
            try
            {
                ...
                List<IEntity<TKey>> items = m_context.Data as List<IEntity<TKey>>;
                items.Remove(items.FirstOrDefault(f => f.StudentID.Equals(imodel.StudentID)));
                items.Add(imodel);
                m_context.Data = items as ICollection<TEntity>;
                return true;
            }
